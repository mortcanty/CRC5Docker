...
#  stretch the pixel vectors to [-1, 1]
    ...
    if (trainalg != 1):
        for j in range(len(pos)):
            Xs[:, j] = 2 * (Xs[:, j] - minx[j]) / (maxx[j] - minx[j]) - 1
...
#  initialize classifier
    if trainalg == 1:
        ...
        classifier = sc.RF(Xstrn, Lstrn, 50, 500)
    # train it
...
#      classify the image tile by tile
        ...
        if tstfile:
            with open(tstfile,'w') as f:
                f.write(algorithm +'test results for %s \n'%infile)
                f.write(time.asctime()+'\n')

                f.write('Classification image: %s \n'%outfile)
                f.write('Class probabilities image: %s \n'%probfile)
                f.write('%s, %s \n'%(Lstst.shape[0],Lstst.shape[1]))

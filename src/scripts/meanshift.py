...
#          update mean         
            m = (np.sum(dta[indices,:],0)/count).astype(int)
...
Options:
  ...
  -------------------------------------'''%sys.argv[0]   
    ...
    start = time.time()                                     
#  input image           
    data = np.zeros((nr,nc,nb+2),dtype=float)
    k = 0
    for b in pos:
...
#  loop over all pixels   
    ...
    while idx<m:    
        ...
        idx_max = int(max(idx_max,cpts_max))
...
#  write to disk   
    ...
    labeled = scipy.ndimage.median_filter(np.reshape(labeled,(nr,nc)),3)

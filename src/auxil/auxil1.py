...
def kernelMatrix(X, Y=None, gam=None, k=0):
    if Y is None:
        Y = X
    if k == 0:
        X = np.mat(X)
        ...
        # d = X^2
        d = np.mat(np.sum(X*X, axis=1)).T*onesn
        # d = X^2 + Y^2
        d = d + onesm.T*np.mat(np.sum(Y*Y, axis=1))
        # d = X^2 + Y^2 - 2XY
        d = d - 2*np.mat(X)*np.mat(Y).T
        if gam is None:
            sigma = np.sum(np.sqrt(abs(d)))/(m**2-m)
            gam = 1/(2*(n*sigma)**2)
        return (np.exp(-gam*d), gam)

...
row3 = widgets.HBox([widgets.VBox([w_preview,w_review,w_reset]),w_changemap,w_bmap,w_view,w_vis,w_masks])
        ...
        try:
            ...
            if w_maskchange.value==True:
                if w_changemap.value=='Frequency':
                    ...
                    mp = mp.updateMask(mp.gt(0))
            M.add_layer(TileLayer(url=GetTileLayerUrl(mp.visualize(min=mn, max=mx,
                                  palette=palette)), name=w_vis.value))
        except Exception as e:
            print('Error: %s'%e)
...
def run():
    ...
    location = geolocator.geocode('Olpe')
    M = geemap.Map(center=(location.latitude, location.longitude),zoom=11)
